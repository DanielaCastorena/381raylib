...
void updateEntitiesPositions(Plane *planes, Ship *ships, Bird *bird, float deltaTime) {
    for (int i = 0; i < 5; i++) {
        UpdatePhysicsComponent(&planes[i].position, &planes[i].velocity, planes[i].heading, deltaTime);
        ships[i].position = planes[i].position;
    }

    UpdatePhysicsComponent(&bird->position, &bird->velocity, 0.0f, deltaTime);
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        updateEntitiesPositions(planes, ships, &bird, deltaTime);
